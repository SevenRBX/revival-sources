...
local mouseLockLookScreenUrl = "https://assetdelivery.roblox.com/v1/asset?id=54071825"
local classicLookScreenUrl = "https://assetdelivery.roblox.com/v1/asset?id=45915798"
...
local function createHelpDialog(baseZIndex)
	...
	buttons[2].Function = function() 
		image.Image = "https://assetdelivery.roblox.com/v1/asset?id=45915811"
	end 
	buttons[3] = {}
	buttons[3].Text = "Gear"
	buttons[3].Function = function() 
		image.Image = "https://assetdelivery.roblox.com/v1/asset?id=45917596"
	end
	buttons[4] = {}
	buttons[4].Text = "Zoom"
	buttons[4].Function = function() 	
		image.Image = "https://assetdelivery.roblox.com/v1/asset?id=45915825"
	end 
...
local function createGameSettingsMenu(baseZIndex, shield)
	...
	if hasGraphicsSlider then
		...
		local function setGraphicsQualityLevel(newLevel)
			...
			if newSetting == 20 then -- Level 20 is the same as level 21, except it doesntt render ambient occlusion
				newSetting = 21
		...
		if false or UserSettings().GameSettings.SavedQualityLevel == Enum.SavedQualitySetting.Automatic then
			if false then
				settings().Rendering.EnableFRM = false
		...
		autoGraphicsButton.MouseButton1Click:connect(function()
			if false and not game.Players.LocalPlayer then return end
		...
		game.Players.PlayerAdded:connect(function(player)
			if player == game.Players.LocalPlayer and false then
		...
		--studioCheckbox.Parent = gameSettingsMenuFrame -- todo: enable when studio h4x arentt an issue anymore
		studioCheckbox:SetVerb("TogglePlayMode")
		studioCheckbox.Visible = false -- todo: enabled when studio h4x arentt an issue anymore
		
		local wasManualGraphics = (settings().Rendering.QualityLevel ~= Enum.QualityLevel.Automatic)
		if false and not game.Players.LocalPlayer then
		...
		if game.CoreGui.Version >= 4 then -- we can use escape!
			game:GetService("GuiService").EscapeKeyPressed:connect(function()
				...
				elseif #lastMenuSelection > 0 then
					...
					if #lastMenuSelection == 1 then -- apparently lua cantt reduce count to 0... T_T
						lastMenuSelection = {}
					end
				else
					resumeGameFunction(shield)
...
local createSaveDialogs = function()
	...
	messageBoxButtons[3] = {}
	messageBoxButtons[3].Text = "Dontt Save"
	...
	errorBoxButtons[buttonOffset+1].Text = "Dontt Save"
	...
	while spinnerNum <= 8 do
	   ...
	   spinnerImage.Image = "https://assetdelivery.roblox.com/v1/asset?id=45880710"
			...
			while spin do
				...
				while pos < 8 do
					if pos == spinPos or pos == ((spinPos+1)%8) then
						spinnerIcons[pos+1].Image = "https://assetdelivery.roblox.com/v1/asset?id=45880668"
					else
						spinnerIcons[pos+1].Image = "https://assetdelivery.roblox.com/v1/asset?id=45880710"
					end
		...
		function(enterPressed)
			if enterPressed then
				if chatBox.Text ~= "" then
					...
					if string.sub(str, 1, 1) == t%t then
...
-- TODO: remove click to walk completely if testing shows we dontt need it
