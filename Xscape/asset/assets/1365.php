...
-- 
-- Return:
-- cellPos - The terrain cell intersection point if there is one, vectorPos if there isntt.
...
function GetTerrainForMouse(mouse)
	...
	-- Check for a plane intersection.  If there isntt one then nothing will get hit.
...
local function rotatePartAndChildren(part, rotCF, offsetFromOrigin)
	-- rotate this thing, if itts a part
...
local function setSeatEnabledStatus(model, isEnabled)
	...
	if isEnabled then
		...
		--   this tricks it into thinking therets already someone sitting there, and it wontt make you sit XD
		for i = 1, #seatList do
...
local function getBoundingBox2(partOrModel)
	...
	-- relative to the first partts coordinate frame.
...
local function getBoundingBoxInWorldCoordinates(partOrModel)
	...
	elseif partOrModel:IsA("Terrain") then
		-- we shouldntt have to deal with this case
		--minVec = Vector3.new(-2, -2, -2)
		--maxVec = Vector3.new(2, 2, 2)
	else
...
local function isBlocker(part) -- returns whether or not we want to cancel the stamp because wetre blocked by this part
...
-- character is 5 studs tall, wetll check a 1.5 x 1.5 x 4.5 box around char, with center .5 studs below torsocenter
...
local function findConfigAtMouseTarget(Mouse, stampData)
	...
	--				will it work for parts in a model that are not orthogonally aligned with the modelts CF.
	...
	if Mouse then
		-- DONtT WANT THIS IN TERMS OF THE MODEL CFRAME!  (.TargetSurface is in terms of the part CFrame, so this would break, right?  [HotThoth])
...
t.SetupStamperDragger = function(modelToStamp, Mouse, StampInModel, AllowedStampRegion, StampFailedFunc)
	...
	if StampInModel then
		if not StampInModel:IsA("Model") then
			error("SetupStamperDragger: StampInModel (optional third arg) is not of type tModelt")
			return nil
		end
	...
	local function DoHighScalabilityRegionSelect()
		...
		if game.Players["LocalPlayer"] then 
			...
			if gui and gui:IsA("PlayerGui") then
				if HighScalabilityLine.Dimensions == 1 and line.magnitude > 3 then -- dontt show if mouse hasntt moved enough
	...
	local function DoStamperMouseMove(Mouse)
		...
		-- There wasntt a target (no part or terrain), so check for plane intersection.
		...
		-- dontt move with dragger - will move in one step on mouse down
		...
		if autoAlignToFace(stampData.CurrentParts) and targetSurface ~= 1 and targetSurface ~= 4 then -- pre-rotate the flag or portrait so itts aligned correctly
		...
		lastTarget.CFrame = targetCFrame  -- successful positioning, so update tdat cframe
		if stampData.CurrentParts:FindFirstChild("ClusterMaterial", true) then
		...
		if Mouse and Mouse.Target and Mouse.Target.Parent then
			...
			if (true) then
				...
				-- now we have to cast the ray back in the other direction to find the surface wetre stamping FROM
	...
	local function setupKeyListener(key, Mouse)
		if control and control["Paused"] then return end -- dontt do this if we have no stamp
		
		key = string.lower(key)
		if key == trt and not autoAlignToFace(stampData.CurrentParts) then -- rotate the model
		...
		elseif key == tct then -- try to expand our high scalability dragger dimension
	...
	local function prepareModel(model)
		...
		-- since wetre cloning the old model instead of the new one, we will need to update the orientation based on the original value AND how many more
	...
	local function checkTerrainBlockCollisions(cellPos, checkHighScalabilityStamp)
			...
			for b = 1, #cellBlockingParts do
				...
				-----------------------------------------------------------------------------------
						...
						if blockingPersonTorso then
							-- if so, letts push the person upwards so they pop on top of the stamped model/part (but only if therets space above them)
		...
		if not skipThisCell then  -- if we STILL arentt skipping...  then wetre good to go!
			local canSetCell = true
			
			if checkHighScalabilityStamp then -- check to see if cell is in region, if not wetll skip set
	...
	local function ResolveMegaClusterStamp(checkHighScalabilityStamp)
		...
		-- and also get rid of any HighScalabilityLine stuff if itts there
		HighScalabilityLine.Start = nil
	...
	local function DoStamperMouseUp(Mouse)
		...
		-- if unstampable face, then dontt let us stamp there!
		...
		-- HotThothts note:  Now that above CurrentParts positioning has been commented out, to be truly correct, we would need to use the 
		--                     value of configFound from the previous onStamperMouseMove call which moved the CurrentParts
		--                     Shouldntt this be true when lastTargetCFrame has been set and false otherwise?
		configFound, targetCFrame = findConfigAtMouseTarget(Mouse, stampData)
		...
		if configFound and not HighScalabilityLine.Adorn.Parent then
			...
			for b = 1, #blockingParts do
				...
				-- below if we want to block stamping on self and also stamping that intersects a model wetve previously stamped on our baseplate
		...
		elseif (not configFound) and not (HighScalabilityLine.Start and HighScalabilityLine.Adorn.Parent) then -- if no config then only stamp if itts a real HSL!
		...
		if stampData.CurrentParts:IsA("Model") or stampData.CurrentParts:IsA("Tool") then
			if stampData.CurrentParts:IsA("Model") then
				-- Tylerts magical hack-code for allowing/preserving clones of both Surface and Manual Welds...  just dontt ask X<
			...
			-- if itts a model, we also want to fill in the playerID and playerName tags, if it has those (e.g. for the friend-only door)
		...
		-- and make sure we dontt delete it, now that itts not a ghost part
