...
--ChildAdded on Player (if itts name is "leaderstats")
...
if UserSettings and LoadLibrary then
	...
	local function showPlayerMenu(player, x, y)
		...
		if not playerContextMenuHasItems() then return end -- dontt show if we have nothing to show
		...
		if isScore then
			...
			if not statValues or #statValues < numStatColumns then
				...
				if not leaderstats then
					--We dontt even have a leaderstats child, wait for one
				...
				secondButton.MouseEnter:connect(
					function(x,y)
						if playerContextMenu and playerContextMenu.Visible then
							return
						end -- dontt update if we currently see it
...
if contextMenu3d then
	...
	if game.Players.LocalPlayer["HoverOnPlayerChanged"] then
		game.Players.LocalPlayer.HoverOnPlayerChanged:connect(function(otherPlayer)
			if not inMenu then
				if otherPlayer and otherPlayer.userId < 0 then return end -- we dontt want this for guests
			end
			wait(0.5)
