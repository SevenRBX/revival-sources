...
local function ScopedConnect(parentInstance, instance, event, signalFunc, syncFunc, removeFunc)
	...
	--Now connect us if wetre already in the world
	tryConnect()
...
t.CreateSetPanel = function(userIdsForSets, objectSelected, dialogClosed, size, position, showAdminCategories, useAssetVersionId)
	...
	if showAdminCategories == nil then -- by default, dontt show beta sets
		...
		for index = origArrayPos, arrayPosition do
			if insertButtons[index] then
				if contents[index] then
					...
					-- we dontt want water to have a drop down button
	...
	function(funcNameOrFunc) 
		...
		if funcNameOrFunc == "CreatePropertyDropDownMenu" or funcNameOrFunc == t.CreatePropertyDropDownMenu then
			return "Function CreatePropertyDropDownMenu.  " ..
				   ...
				   "Side effect: returns a container with a drop-down-box that is linked to the tpropertyt field of tinstancet which is of type tenumTypet" 
		end 
		if funcNameOrFunc == "CreateDropDownMenu" or funcNameOrFunc == t.CreateDropDownMenu then
			return "Function CreateDropDownMenu.  " .. 
			       "Arguments: (items, onItemSelected).  " .. 
				   "Side effect: Returns 2 results, a container to the gui object and a tupdateSelectiont function for external updating.  The container is a drop-down-box created around a list of items" 
		end 
		if funcNameOrFunc == "CreateMessageDialog" or funcNameOrFunc == t.CreateMessageDialog then
			return "Function CreateMessageDialog.  " .. 
			       "Arguments: (title, message, buttons). " .. 
			       "Side effect: Returns a gui object of a message box with ttitlet and tmessaget as passed in.  tbuttonst input is an array of Tables contains a tTextt and tFunctiont field for the text/callback of each button"
		end		
		if funcNameOrFunc == "CreateStyledMessageDialog" or funcNameOrFunc == t.CreateStyledMessageDialog then
			return "Function CreateStyledMessageDialog.  " .. 
			       "Arguments: (title, message, style, buttons). " .. 
			       "Side effect: Returns a gui object of a message box with ttitlet and tmessaget as passed in.  tbuttonst input is an array of Tables contains a tTextt and tFunctiont field for the text/callback of each button, tstylet is a string, either Error, Notify or Confirm"
		end
		...
		if funcNameOrFunc == "CreateScrollingFrame" or funcNameOrFunc == t.CreateScrollingFrame then
			return "Function CreateScrollingFrame.  " .. 
			   ...
			   "Side effect: returns 4 objects, (scrollFrame, scrollUpButton, scrollDownButton, recalculateFunction).  tscrollFramet can be filled with GuiObjects.  It will lay them out and allow scrollUpButton/scrollDownButton to interact with them.  Orderlist is optional (and specifies the order to layout the children.  Without orderlist, it uses the children order. style is also optional, and allows for a tgridt styling if style is passed tgridt as a string.  recalculateFunction can be called when a relayout is needed (when orderList changes)"
		end
		if funcNameOrFunc == "CreateTrueScrollingFrame" or funcNameOrFunc == t.CreateTrueScrollingFrame then
			return "Function CreateTrueScrollingFrame.  " .. 
			   "Arguments: (nil) " .. 
			   "Side effect: returns 2 objects, (scrollFrame, controlFrame).  tscrollFramet can be filled with GuiObjects, and they will be clipped if not inside the framets bounds. controlFrame has children scrollup and scrolldown, as well as a slider.  controlFrame can be parented to any guiobject and it will readjust itself to fit."
		end
		if funcNameOrFunc == "AutoTruncateTextObject" or funcNameOrFunc == t.AutoTruncateTextObject then
			return "Function AutoTruncateTextObject.  " .. 
			   "Arguments: (textLabel) " .. 
			   "Side effect: returns 2 objects, (textLabel, changeText).  The ttextLabelt input is modified to automatically truncate text (with ellipsis), if it gets too small to fit.  tchangeTextt is a function that can be used to change the text, it takes 1 string as an argument"
		end
		if funcNameOrFunc == "CreateSlider" or funcNameOrFunc == t.CreateSlider then
			return "Function CreateSlider.  " ..
				"Arguments: (steps, width, position) " ..
				"Side effect: returns 2 objects, (sliderGui, sliderPosition).  The tstepst argument specifies how many different positions the slider can hold along the bar.  twidtht specifies in pixels how wide the bar should be (modifiable afterwards if desired). tpositiont argument should be a UDim2 for slider positioning. tsliderPositiont is an IntValue whose current .Value specifies the specific step the slider is currently on."
		end
